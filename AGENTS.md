...
- **Provider facades** — removed. Do not add compatibility shims for deleted hosting facades, MCP wrappers, data-ingestion helpers, declarative workflow experiments, Durable generator experiments, or product-host samples.
...
- **MAF version discipline**: bump the remaining `Microsoft.Agents.AI.*` pins in `Version.props` as a group. The active toolkit is on the stable 1.8.0 line: `Microsoft.Agents.AI`, `Microsoft.Agents.AI.Abstractions`, and `Microsoft.Agents.AI.Workflows`.
