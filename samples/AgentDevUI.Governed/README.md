...
through `.AsBuilder().UseOpenTelemetry(...).UseQylGovernance(...)`: `UseOpenTelemetry()` wraps the agent
in MAF's `OpenTelemetryAgent` (conformant gen_ai `invoke_agent` / `execute_tool` spans, sensitive data
off by default and pinned off here), and `UseQylGovernance(...)` inserts capability, budget, and
concurrency enforcement around every tool invocation. The framework's spans/metrics are collected on
