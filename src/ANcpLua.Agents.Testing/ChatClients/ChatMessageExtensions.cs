...
public static class ChatMessageExtensions
{
    ...
    public static IReadOnlyList<AIContent> ToContentStream(this string? message)
    {
        ...
        return splits.Select<string, AIContent>(static text => new TextContent(text) { RawRepresentation = text })
            .ToList();
    }
    ...
    public static ChatMessage ToChatMessage(
        ...
        string? authorName = null)
    {
        return new ChatMessage(ChatRole.Assistant, contents is List<AIContent> list ? list : contents.ToList())
        {
            AuthorName = authorName,
            CreatedAt = TimeProvider.System.GetUtcNow(),
            MessageId = messageId ?? Guid.NewGuid().ToString("N"),
    ...
    public static IReadOnlyList<ChatMessage> ToChatMessages(
        ...
        string? authorName = null)
    {
        return messages.Select(text => new ChatMessage(ChatRole.Assistant, text.ToContentStream().ToList())
        {
            AuthorName = authorName,
            MessageId = Guid.NewGuid().ToString("N"),
