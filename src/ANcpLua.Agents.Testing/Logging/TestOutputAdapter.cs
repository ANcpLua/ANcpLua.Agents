...
namespace ANcpLua.Agents.Testing.Logging;
...
public sealed class TestOutputAdapter(ITestOutputHelper output) : TextWriter, ILogger, ILoggerFactory
{
        ...
        Func<TState, Exception?, string> formatter)
    {
        ...
        var scope = _scopes.Count > 0 ? $"[{_scopes.Peek()}] " : string.Empty;
    ...
    /// </summary>
    public IReadOnlyList<LogRecord> GetCapturedLogs()
    {
    ...
    {
        return [.. _capturedLogs.Where(r => r.Level == level)];
    }
    ...
    private void SafeWrite(string value)
    {
        ...
        catch (InvalidOperationException ex) when (ex.Message.ContainsOrdinal("no currently active test"))
        {
