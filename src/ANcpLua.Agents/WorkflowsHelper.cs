...
public static class WorkflowsHelper
{
        ...
        await foreach (var evt in run.WatchStreamAsync())
        {
            switch (evt)
            {
                ...
                case ExecutorInvokedEvent invoked:
                    ...
                    if (!executorTimings.TryGetValue(invoked.ExecutorId, out var invokedStopwatch))
    ...
    private static void PrintToolCalls(IReadOnlyList<FunctionCallContent> calls)
    {
        ...
        {
            ColorHelper.PrintColoredLine(
                $"[TOOL CALL: {call.Name}] with arguments: {JsonSerializer.Serialize(call.Arguments)}",
                ConsoleColor.Blue);
    ...
    private static void PrintWorkflowOutput(WorkflowOutputEvent output)
    {
        ...
        if (!string.IsNullOrWhiteSpace(final)) ColorHelper.PrintColoredLine(final, ConsoleColor.Yellow);
    ...
    private static void PrintExecutionSummary(Dictionary<string, TimeSpan> executorDurations, TimeSpan totalTime)
    {
        ...
        if (executorDurations.Count is not 0)
        {
            ...
            foreach (var kvp in executorDurations.OrderByDescending(static x => x.Value))
            {
                ...
                Console.WriteLine(
                    $"  {executorName}: {kvp.Value.TotalSeconds:F2}s ({kvp.Value.TotalMilliseconds:F0}ms)");
            }
        ...
        Console.WriteLine(
            $"TOTAL Workflow Execution Time: {totalTime.TotalSeconds:F2}s ({totalTime.TotalMilliseconds:F0}ms)");
        Console.WriteLine(
            "***** End Summary *************************************************************************************************");
        Console.WriteLine();
    }
