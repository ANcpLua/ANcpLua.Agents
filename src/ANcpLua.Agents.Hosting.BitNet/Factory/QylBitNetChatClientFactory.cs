...
/// </summary>
public static class QylBitNetChatClientFactory
{
    private const string UnusedApiKey = "unused";

    /// <summary>
    ///     Constructs an <see cref="IChatClient" /> from <paramref name="options" />. Calls
    ///     <see cref="QylBitNetClientOptions.Validate" /> first so misconfiguration fails fast at
    ///     composition time.
    /// </summary>
    ...
    public static IChatClient Create(QylBitNetClientOptions options)
    {
    ...
    /// <summary>
    ///     Convenience overload: reads <see cref="QylBitNetClientOptions" /> defaults plus
    ...
    public static IChatClient? TryCreateFromEnvironment()
    {
        ...
        return options.Endpoint is null ? null : BuildChatClient(options);
    }

    private static IChatClient BuildChatClient(QylBitNetClientOptions options)
    {
