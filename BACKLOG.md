...
- **RunQylToolAsTaskAsync integration test** — `McpClient.CallToolAsTaskAsync`/`PollTaskUntilCompleteAsync`/`GetTaskResultAsync` are non-virtual on the abstract `McpClient`, so a unit-mock approach doesn't work. Needs a `WebApplicationFactory<>` test harness + the canonical `AddMcpServer().WithHttpTransport().WithToolsFromAssembly()` chain + `MapQylMcp` + `WithInMemoryTaskStore` + a tool declared with `TaskSupport.Required` + `IProgress<ProgressNotificationValue>`. Validates the full progress-notification-to-OTel-span-event mapping end-to-end. The error-path smoke test (`QylStdioMcpClientSmokeTests`) ships in this commit; the round-trip integration test is the gap.
